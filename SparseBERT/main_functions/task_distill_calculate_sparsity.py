...
# fh = logging.FileHandler('/home.local/dongkuan/Working/Pretrained-Language-Model/TinyBERT/run_sh/debug_layer_loss.log')
...
def main():
    ...
    print('args.student_model: ', args.student_model)
    ...
    for k, v in student_model.named_parameters():
        ...
        #     tmp = v.data.cpu().numpy()
        #     print('k, v.shape, tmp[0:2,0:5]: ', k, v.shape, tmp[0:2,0:5])
        #     #print()
        #     #print('v[0,0:50]: ', v[0,0:50])
        #     print('np.flatnonzero(tmp[0:2,0:5]): ', np.flatnonzero(tmp[0:2,0:5]))
        #     #total_nonezero = total_nonezero + np.flatnonzero(temp).size

        temp = v.data.cpu().numpy()
        total_param = total_param + temp.size
        total_nonezero = total_nonezero + np.flatnonzero(temp).size
        layer_sparse_rate[k] = 1 - np.flatnonzero(temp).size / temp.size
        print('layer, size, sparsity: ', k, v.shape, layer_sparse_rate[k])
        # if '.attention.self.query.weight' in k:
        #     print('k, temp[0:4,0:30]', k, temp[0:4,0:30])
    ...
    if args.do_eval:
        ...
        #     if 'intermediate.dense.weight' in k or 'output.dense.weight' in k:
        #         prune_dict[k] = 0.875

        # # prune = Prune(model, step * 0, step * 8, 10, prune_dict)
        # prune = Prune(student_model, num_train_optimization_steps * 1, num_train_optimization_steps * 4, 20, prune_dict, False, False, 'none')
        # ###############################
